...
echo [3/6] Creating desktop shortcut...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut([Environment]::GetFolderPath('Desktop') + '\FocusMed.lnk'); $s.TargetPath = 'C:\Program Files\FocusMed\FocusMed.Launcher.exe'; $s.WorkingDirectory = 'C:\Program Files\FocusMed'; $s.Description = 'FocusMed Medical Imaging'; $s.Save()"

echo [4/6] Creating Start Menu shortcut...
...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut([Environment]::GetFolderPath('ApplicationData') + '\Microsoft\Windows\Start Menu\Programs\FocusMed\FocusMed.lnk'); $s.TargetPath = 'C:\Program Files\FocusMed\FocusMed.Launcher.exe'; $s.WorkingDirectory = 'C:\Program Files\FocusMed'; $s.Description = 'FocusMed Medical Imaging'; $s.Save()"

echo [5/6] Setting up autostart and firewall...
...
echo.
echo Desktop shortcut created: FocusMed
echo Autostart enabled: FocusMed will start on login
echo.
