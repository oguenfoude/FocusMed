...
92. **localhost:5000 buttons dead, IP:5000 works — IPv4/IPv6 mismatch.** `Program.cs` now calls `builder.WebHost.UseUrls("http://0.0.0.0:5000")`. Without it, `0.0.0.0` binds IPv4-only; Chrome resolves `localhost` to `::1` (IPv6) so the Blazor Server SignalR circuit never connects and clickable elements do nothing. The Launcher opens the dashboard via `http://127.0.0.1:{port}` (not `localhost`) in `App.xaml.cs` (both single-instance relaunch and `OpenDashboard`) to avoid the dual-stack issue entirely. Symptom on client PCs: page loads, but every button is inert.
