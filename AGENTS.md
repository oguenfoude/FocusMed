...
68. **Duplex checkbox defaults to OFF** (`StudyDetails.razor:407`). `PrintDuplex` is reset to `false` in `OpenPrintDialog()` every time the modal opens. A visible label "OFF par defaut" appears next to the checkbox. When `CanDuplex=false`, the checkbox is disabled with a "(non disponible)" tooltip. The `PrintRequest.Duplex` field is a simple `bool` — booklet mode uses `Duplex.Vertical` internally regardless of the checkbox state.
...
- `src/FocusMed.PrintService/Services/BookletImpositionService.cs` — `IBookletImpositionService` implementation. Uses PdfSharpCore to reorder pages into booklet signature order (N padded to multiple of 4). Writes imposed PDF to temp file.
