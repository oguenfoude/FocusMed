...
75. **Tests: `src/FocusMed.Tests`** (xUnit, `net10.0-windows`, in `FocusMed.slnx`). Covers `DicomHelpers` (FormatPatientName/SanitizeFileName/IsSubdirectoryOf/FNV-1a/GetDicomDate incl. the PN-tag trick — DA VR rejects invalid strings on write so invalid-input cases go through `PatientName`), `PdfService.ComputeGridCols` (hand-verified pinned values: 4→2, 6→2 cols; bounds 1..count), and the `VACUUM INTO` backup technique (temp file DB → copy → row verified). Run: `dotnet test src/FocusMed.Tests`. CI runs it with `--no-build` after `dotnet build -warnaserror`.
