...
27. **N-SET patient resolution chain** (`FocusMedScp.cs:807-896`): (1) FK chain: `imageBox→FilmBox→PrintJob→Patient`, (2) inner DICOM dataset: `PatientID`/`PatientName` from `BasicColorImageSequence`/`BasicGrayscaleImageSequence`, (3) **OriginalImageSequence (2130,00C0)** — Annex H carries mandatory PatientID + Study/Series UIDs there, (4) **top-level `request.Dataset`** `PatientID`/`PatientName`, (5) **FilmSession fallback** (`_incomingPatientId`/`_incomingPatientName` captured at N-CREATE), else empty string. The former "most recent C-STORE study" fallback was **removed** — it risked cross-patient PHI contamination. Resolution is logged at Information with the source tag ("PrintJob"/"InnerDataset"/"OriginalImageSequence"/"TopLevelDataset"/"FilmSession"/"none") so identity flow is verifiable. Never generates UNKNOWN. **BirthDate/Sex ride along too**: FilmSession N-CREATE captures `PatientBirthDate`/`PatientSex` from `ProposedStudySequence` (2130,00A0) or the top-level dataset, injects them into the inner N-SET dataset when the image carries none, and `IngestPrintImageAsync` writes them onto the `Patient` entity (same as the C-STORE path). The ingest log shows `birth='...' sex='...'`.
...
35. **PNG extraction is lazy AND pre-warmed at completion.** PNGs are extracted on-demand when a viewer calls `GetOrExtractFramesAsync(studyId)`, AND pre-extracted once by `StudyCompletionService` at completion via `PngExtractionService.PreExtractStudyAsync(studyId)` (fire-and-forget, refcount released inside, failures non-fatal). So a completed study opens instantly (no DICOM→PNG render on first view); a study still Receiving falls back to on-demand extraction. Extraction stays out of the C-STORE receive path (no CPU spike during receipt). PNGs persist on disk permanently once extracted.
...
51. **PdfService cover generation is QuestPDF-only** (`PdfService.cs`). The old Word COM chain (`ConvertDocxToPdfViaWord`, WINWORD.EXE spawn, 1-3s, Word install required) was **removed** — QuestPDF renders the cover directly in ~100ms with no external processes. `GenerateCoverPdfUncached` is a straight pass-through to `GenerateCoverPdfFallback` (the name is historical). `cover.docx` no longer exists (deleted from `wwwroot/`, no longer provisioned by `Program.cs`); only `cover-logo.jpg` is still provisioned and used as the cover background. Do not reintroduce COM-based cover generation.
...
60. **There is NO `_mergeLocks` — only `_studyLocks` + `_studyLockCounts`** (`DicomUpsertService.cs:21-22`). An earlier version of this entry claimed a separate merge-lock dictionary existed; it never did. Forward merge (`MergeRecentAnonymousStudiesAsync`) runs **inside** the per-study `_studyLock`, so it is serialized against inserts into the same target study. Reverse merge (`StudyCompletionService.MergeRecentPrintsIntoStudyAsync`) runs unlocked in the completion loop. Cross-study merge collisions are prevented by the patient/CallingAE filters, not by locking — do not add a global merge lock without measuring first.
...
93. **C-STORE re-send visibility: `StoreOutcome` + UID fork + no silent dedup** (`DicomUpsertService.cs`, `FocusMedScp.cs`). An identical re-send (same SOP UIDs) used to hit a bare `return` with zero logging — sender saw Success, DB + logs + chime stayed silent, operator thought the send vanished. Now **every C-STORE produces a visible row**: same-SOP re-send within the same study forks the SOP UID (`.guid` suffix) and stores it; cross-study SOP collision also forks. `StoreFileOnlyAsync` returns `StoreOutcome` (`Stored`/`SopCollisionForked`/`DedupedSameStudy` — the last is retained but never fires in normal flow). Duplicate sends log `C-STORE same-SOP re-send SOP=... forked` at Information. The completion-loop reopen logic runs **after** the dedupe check, so re-sending a Complete study both forks the image AND reopens to `Receiving`. `FocusMedScp` keeps per-association counters and logs `Association C-STORE summary AE=...: N stored, M deduped, K failed` on release/close. Dashboard `MainLayout` shows `Dernière réception HH:mm:ss` from `MAX(LastUpdatedAt)` in the existing 2s poll; Home rows carry a `title` tooltip with last-activity time. Tests pin both cases (`Ingest_SameSopUidTwice_CreatesTwoImages`, `Ingest_ResendAfterComplete_CreatesSecondRow`).
...
87. **One study row per distinct StudyInstanceUID — NEVER auto-merge across exams** (`DicomUpsertService`, `StudyCompletionService`). Design rule (2026-09-13, replaces the old patient-consolidation rule): two different exams for the same patient, even on the same day, ALWAYS become separate studies. C-STORE (`StoreFileOnlyAsync`) resolves the target study by UID match only (any non-`Deleted` status) inside the per-UID lock — no pre-lock resolve scope, no patient-based reuse, no UID fork. Same UID arriving late (after `Complete`/`Archived`) appends to the same study and reopens it to `Receiving`; `Deleted` studies never resurrect (re-send creates a fresh row). Empty incoming Study UID (degenerate modalities) groups narrowly: patient's most recent `Receiving` minted-empty study, same `CallingAeTitle`, 5-min window — else a fresh UID. Prints (`IngestPrintImageAsync`, now takes optional `printJobId`) resolve: (a) `PrintJob.StudyId` already linked (N-SET #2..N rejoin N-SET #1's study — the N-SET handler passes `imageBox.FilmBox?.PrintJob?.Id` and links `StudyId` after ingest), (b) explicit `StudyInstanceUID` in the dataset (OriginalImageSequence/ProposedStudySequence) matching a non-`Deleted` study, (c) else a brand-new study. Patient/AE heuristics were DELETED. Both auto-merge methods were DELETED: `ResolveStoreTargetUidAsync`, `MergeRecentAnonymousStudiesAsync` (forward), `MergeRecentPrintsIntoStudyAsync` (reverse) — no callers remain. `PrintMergeWindowSeconds` config + `_printMergeWindowSeconds` fields removed from both services (the `DicomNetworkingOptions` property itself stays). Manual merge (`StudyService.MergeStudyAsync`, gotcha 89) is the operator's explicit escape hatch. UIDs sanitized via `TruncateUid` (digits + dots only, max 64 chars); minted UIDs are `2.25.*` (UUID arc). New print Series are tagged `Modality = "SC"`. Per-study `_studyLocks` keyed on the incoming/adopted UID (same-UID sends serialize, different UIDs run parallel). Tests: `UpsertIngestTests` pins separation (`DifferentStudyUidSamePatient_CreatesTwoStudies`), same-UID pieces (`SameStudyUidTwoSeries_OneStudy`), print job grouping, explicit-UID link, and print-separation.

88. **`CallingAeTitle` is recorded on ingest.** `Study.CallingAeTitle` (`Study.cs:14`) and `PrintJob.CallingAeTitle` (`PrintJob.cs:10`) are nullable strings set from `Association.CallingAE` — C-STORE (`StoreFileOnlyAsync`) and print N-CREATE/N-SET. New columns indexed (`Study.CallingAeTitle`, composite `PrintJob.{CallingAeTitle,CreatedAt}`) in migration `PrintMergeSupport`. Used by the FilmBox fallback (gotcha 30) and the empty-UID grouping grace (gotcha 87). Never empty-string — null when unknown.
...
**Note:** `deploy/` is the live single-folder deployment (Worker + Dashboard + Launcher + merged `appsettings.json` + `config.json`) — republished and E2E smoke-tested (Worker binds 11112, Dashboard serves :5000 `/health` ok, migrations + metadata backfill ran). Publish model is **self-contained `win-x64`** (`dotnet publish -c Release -r win-x64 --self-contained true -o deploy`), so target PCs need **no** .NET installed — proven by booting both exes with a bogus `DOTNET_ROOT` + `DOTNET_MULTILEVEL_LOOKUP=0`. To republish after code changes (preserve merged appsettings + config.json): back up `deploy/appsettings.json` and `deploy/config.json`, kill all 3 exes, then `dotnet publish` the Worker, Dashboard, Launcher csproj `-c Release -r win-x64 --self-contained true -o deploy` in that order, then restore both files. **Every csproj has a `RemoveAppLocalHostFx` publish target** (conditioned on `SelfContained != true`) — framework-dependent .NET 10 publish drops `hostfxr.dll`/`hostpolicy.dll` into the output and the published exe then prefers those app-local hosts and dies with `No frameworks were found` (even when .NET 10 is installed machine-wide, on the same Windows box the `dotnet exec` path works fine). The target deletes them in that mode; in self-contained mode the app-local hosts + bundled runtime are kept. Also QA-only: publish sweeps in junk (per-RID `libSkiaSharp.pdb`, `runtimes/linux-*`/`osx-*`/`win-arm64`/`win-x86` natives, `BuildHost-*`, `package*.json`) — the installer `Excludes` them (**comma-separated** — semicolons silently match nothing, verified 2026-09-08 when a `;`-joined list packed the setup into itself: 176 MB); for a lean deploy delete those folders manually after publish. A copy of the final `FocusMedSetup-1.0.0.exe` lives in `deploy/` too so the folder is the complete shareable package (also excluded from the installer payload via `FocusMedSetup-*.exe`). The old `Install.bat`/`Uninstall.bat`/`README.txt` xcopy flow was removed from `deploy/` (superseded by the Inno installer); `Diagnostic.bat` stays as a client support tool. Run the whole flow via `& "D:\FocusMed\deploy\FocusMed.Launcher.exe"` in an **Administrator** PowerShell — it supervises Worker + Dashboard and runs the resume print monitor. Do NOT run dev Worker/Dashboard (`dotnet run`) at the same time as the launcher (port 11112/5000 conflict + double restart supervision). `deploy/` is gitignored.
...
66. **Arrival chime on DICOM receive** (`MainLayout.razor`, `wwwroot/js/notification.js`, `wwwroot/sounds/notification.wav`). `MainLayout` polls `MAX(DicomImages.Id)` every 2s via `IDbContextFactory` — every C-STORE and every print N-SET inserts image rows (merges/completions/deletes do not), so a jump means new DICOM arrived and the 880 Hz / 0.8 s WAV chime plays via JS `Audio`. Cross-process by design: the Worker's in-memory `StudyNotificationService` singleton never reaches the Dashboard process, so subscribing to `OnStudyChanged` in the Dashboard would never fire — the DB poll is the signal. First poll sets a silent baseline (no chime on page load). Mute toggle (speaker button in navbar, `opacity:.35` inline style when muted — no new Tailwind classes so no CSS rebuild needed) persists in `localStorage` (`focusmed-sound-muted`); a 3 s `localStorage` cross-tab debounce (`focusmed-last-chime`) prevents double chimes with two tabs open. `window.__focusmedLastChime` is a permanent test hook (Playwright verified: probe row → chime timestamp set). To replace the sound, drop any `.wav`/`.mp3` over `wwwroot/sounds/notification.wav`. No config keys, no Settings page, no Launcher/Worker changes. `MainLayout` reads `DicomNetworking:DicomPort` (`MainLayout.razor:114`) for the TCP health probe — that key must stay in Dashboard `appsettings.json` (dev) and the merged config (prod).

67. **`PdfService.GeneratePrintPdf` is async** (`PdfService.cs:38-50`). It `await`s `CleanupOldPdfsAsync()` and `pdfLock.WaitAsync(ct)` — never `GetAwaiter().GetResult()` / `Wait()` (that sync-over-async risked Blazor thread-pool starvation). Takes an optional `CancellationToken` (StudyDetails passes its regen token). `GenerateBookletPrintPdf` / `GenerateFlatPrintPdf` stay sync — they have no locks/cleanup and already run inside `Task.Run` at the call sites. Cancelled waits use `AbandonPdfLockRef` (decrements refcount WITHOUT `Release()`) — releasing an un-acquired semaphore would corrupt the count and break mutual exclusion for that hash.

73. **All recursive dir deletions MUST pass `DicomHelpers.IsSubdirectoryOf`** (`DicomHelpers.cs`). True only when the target is strictly inside the root (never root itself or above). Reason: `DeletedCleanupService` once chained TWO `GetParent` calls (`dir` → `seriesDir` → `studyDir`), which resolved to `archive/` itself — the first auto-delete past the 30-day threshold would have wiped every study's DICOM files (fixed 2026-09-08; never fired because the system was only 18 days old). Correct depth: FilePath = `archive/<study>/<series>/<file>.dcm`, so the study dir is exactly ONE `GetParent` above `GetDirectoryName(FilePath)`. Manual permanent delete (`StudyService.DeleteStudyAsync`), auto-delete (`DeletedCleanupService`), and retention (`StudyCompletionService.CleanupOldArchivesAsync`) all delete the same four things now: DB rows + archive dirs + PNG dirs (via `DicomImage.PngPath`) + resume PDF + orphan `Patient` rows. Keep the three paths consistent when touching any of them.

68. **StudyDetails dispose guards** (`StudyDetails.razor`). `_disposed` flag (set first in `Dispose()`), `_regenLock` around the `_regenCts` cancel/dispose/renew swap, `_disposed`/cancellation re-checks after every await in `RegeneratePdfPreviewAsync` and `ExtractImagesAsync`. Extraction has its own `_extractCts` (cancelled in `Dispose`) threaded into `GetOrExtractFramesAsync` (which already accepted a CT). `OperationCanceledException` from extraction is swallowed quietly — navigation-away is not an error.

69. **`ArchiveRetentionDays` config** (Worker `appsettings.json`, default **0 = keep forever**). When > 0, `StudyCompletionService` deletes Archived/Deleted studies older than N days: DB rows (cascade to Series/Images/Frames) + archive dirs + PNG dirs (via `DicomImage.PngPath`) + resume PDF + orphan `Patient` rows (same as manual permanent delete). All dir deletions pass `DicomHelpers.IsSubdirectoryOf` (see #73). Default disabled so current behavior is unchanged — set it only when the clinic wants automatic disk reclamation.

70. **Storage Commitment dedups on `TransactionUid`** (`FocusMedScp.cs`). `OnNActionRequestAsync` checks `AnyAsync(j => j.TransactionUid == transactionUid)` before inserting — a retried N-ACTION no longer creates a duplicate job (which would have sent a duplicate N-EVENT-REPORT). No unique DB index (avoids a migration for a low-traffic path).

71. **Installer excludes `appsettings.json`** (`FocusMed.iss:37`). Fresh installs have NO `appsettings.json` until the Launcher generates the merged one via `BootstrapService.EnsureSharedAppSettings()` (runs synchronously before any child spawns) — this removes the nondeterministic "which of the 3 published appsettings wins" state between install and first launch. `config.json` IS packaged (generation source). `deploy/` still keeps its merged `appsettings.json` for xcopy-style use.

72. **Silent-catch sweep (2026-09-08).** All empty `catch { }` / `catch (OperationCanceledException) { }` in Launcher (`BootstrapService`, `ChildProcessSupervisor`, `ResumePickerWindow`), Dashboard `StudyService`, and Dicom `PngExtractionService` now log at Debug (shutdown cancellation) or Warning (cleanup failures). Behavior unchanged — failures are just visible now. `StorageForwardService`/`RawPrintService` already logged and were left alone. The 3 Printing capability-provider registrations are all live (fallback chain in `PrinterCapabilityService`) — not dead code.

74. **Dashboard has file logging** (`Program.cs`). It runs as a HIDDEN Launcher child, so console output went nowhere — Serilog (`Serilog.AspNetCore` + Console + File 10.0.0/6.1.1/7.0.0, same versions as Worker) writes `logs/dashboard-.log` (daily roll, 14 retained) into the data dir, plus `UseSerilogRequestLogging()` for per-request method/path/status/timing. `Routes.razor` wraps the router in an `<ErrorBoundary>` with a French fallback + reload button (inline styles only — must render even if `app.css` fails). `/pdf-cache` is served with `Cache-Control: no-store` — preview URLs are content-hash stable, so a cached 404 (TTL sweep, old tab) would otherwise poison the preview permanently for that tab; local serving makes re-downloads free. `/images` stays cached (PNGs are immutable).

75. **Tests: `src/FocusMed.Tests`** (xUnit, `net10.0-windows`, in `FocusMed.slnx`). Unit: `DicomHelpers` (FormatPatientName/SanitizeFileName/IsSubdirectoryOf/FNV-1a/GetDicomDate incl. the PN-tag trick — DA VR rejects invalid strings on write so invalid-input cases go through `PatientName`), `PdfService.ComputeGridCols` (hand-verified pinned values: 4→2, 6→2 cols; bounds 1..count), and the `VACUUM INTO` backup technique (temp file DB → copy → row verified). Integration (`TestInfra`, sequential — parallelization disabled assembly-wide because tests mutate process-wide `FOCUSMED_DATA`): `UpsertIngestTests` (real `DicomUpsertService` + temp SQLite + temp archive: ingest creates rows+file, re-send dedupes, second modality merges to one study), `AssignResumeTests` (real `DatabaseService`: Complete assigns, Deleted/Archived/missing refused), `CoverPdfTests` (real `PdfService.GeneratePrintPdf` with an ImageSharp PNG and NO cover.docx present: valid multi-page PDF, same-input cache hit). Fixture sets the QuestPDF Community license (mirrors Dashboard `Program.cs`) and `NU1903` is suppressed (same native SQLite production ships). Run: `dotnet test src/FocusMed.Tests`. CI runs it with `--no-build` after `dotnet build -warnaserror`.

76. **Backup: `BackupService`** (Worker only, via `AddFocusMedDicom`). Daily `VACUUM INTO` snapshot (consistent under WAL + concurrent readers — never a raw file copy) to `<dataDir>/backups/focusmed-YYYYMMdd-HHmmss.db`, pruned to `RetentionDays`. Config `Backup:{Enabled=true, IntervalHours=24, RetentionDays=7, Folder=backups}` in Worker `appsettings.json` + tunable in prod via `config.json` (`BackupEnabled/BackupIntervalHours/BackupRetentionDays` in `SiteConfig`, emitted into the merged appsettings by `BootstrapService`). First backup 2 min after startup. VACUUM INTO needs a filename LITERAL — the SQL is plain-string concatenation, never an interpolated FormattableString (EF would parameterize it and SQLite rejects that).

77. **CI (`.github/workflows/ci.yml`).** `build-test` on every push/PR: `dotnet build -warnaserror` + `dotnet test`. `installer` on main only: self-contained publish ×3, junk sweep (same exclusions as local flow), `choco install innosetup`, ISCC compile, setup exe uploaded as artifact.

78. **Regen cancellation always resets `IsRegeneratingPdf`** (`StudyDetails.razor:701,726`). Both the post-`Task.Run` cancellation check AND the `OperationCanceledException` catch reset the flag. Previously either path left the UI in a permanent "Régénération…" spinner with the print button disabled. `LoadStudyAsync` is wrapped in try/catch + finally — DB failure shows `ImageError` instead of a permanent spinner.

79. **Resume picker Confirm has a reentrancy guard** (`ResumePickerWindow.xaml.cs`). `_confirming` bool + button disable on first click; both reset on every failure path (copy error, assign-false, exception). Success closes the window so no reset needed. `AssignResumeAsync` re-validates study state at assign time — Deleted/Archived studies are refused (`DatabaseService.cs`).

80. **Picker shows today + yesterday, UTC window** (`DatabaseService.cs:27-43`). The old "today only via `DateTime.Now.Date.ToUniversalTime()`" filter showed an empty list for after-midnight prints. Now `CreatedAt >= DateTime.UtcNow.Date.AddDays(-1)` — no local-time conversion, no midnight gap. Read-only queries use `.AsNoTracking()`.

81. **Print temp cleanup + STA timeout are logged** (`PrintExecutionService.cs`). `DeleteTempPdf` helper logs at Warning when deletion fails (all three print paths). STA timeout logs at Error before throwing (the abandoned background thread can't be aborted — `Thread.Abort` is unsafe with COM/XPS — it dies with the process). `RunInStaThread` is an instance method (needs the logger). Print-on-navigation-away continuing to completion is intentional — paper already moving can't be un-printed; the spooler owns the job after `Write()`.

82. **PdfService cleanup paths log at Debug** (`PdfService.cs`). All five former silent `catch {}` (temp-file sweeps, PNG aspect probe, TTL sweep) now log. `EstimateImageAspect` and `DeleteExpired` are instance methods (were static — needed the logger). `DicomUpsertService` study-create uses `await SaveChangesAsync()` (was sync).
