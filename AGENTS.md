...
93. **C-STORE re-send visibility: every send = new study, no SOP fork** (`DicomUpsertService.cs`, `FocusMedScp.cs`). `StoreFileOnlyAsync` always generates a fresh `StudyInstanceUID` — the sender's UID is discarded. No UID lookup, no append, no collision fork. Every C-STORE produces a new study row with new images. The `StoreOutcome` enum has only `Stored` (no `SopCollisionForked` or `DedupedSameStudy`). `FocusMedScp` keeps per-association counters and logs `Association C-STORE summary AE=...: N stored, K failed` on release/close. Dashboard `MainLayout` shows `Dernière réception HH:mm:ss` from `MAX(LastUpdatedAt)` in the existing 2s poll; Home rows carry a `title` tooltip with last-activity time. Tests: `SameSopUidTwice_CreatesTwoStudies`, `ResendAfterComplete_CreatesNewStudy`.
