...
93. **C-STORE grouping: same StudyInstanceUID = one study, no reopen on Complete** (`DicomUpsertService.cs`, `FocusMedScp.cs`). `StoreFileOnlyAsync` uses the sender's StudyInstanceUID to group images from the same exam. Empty UID → minted fresh. If the matching study is `Receiving`/`Failed`, images append. If `Complete`/`Archived`, a NEW study is created (re-send = new exam, no reopen). `StoreOutcome` has only `Stored`. `FocusMedScp` keeps per-association counters and logs `Association C-STORE summary AE=...: N stored, K failed` on release/close. Dashboard `MainLayout` shows `Dernière réception HH:mm:ss` from `MAX(LastUpdatedAt)` in the existing 2s poll; Home rows carry a `title` tooltip with last-activity time. Tests: `SameStudyUidTwoSeries_OneStudy`, `DifferentStudyUidSamePatient_CreatesTwoStudies`, `ResendAfterComplete_CreatesNewStudy`.
...
87. **One study row per StudyInstanceUID — never reopen Complete/Archived** (`DicomUpsertService`, `StudyCompletionService`). Design rule: images from the same exam share the same StudyInstanceUID → one study row. If that study is already Complete/Archived when a re-send arrives, a NEW study is created (re-send = new exam, not a reopen). C-STORE (`StoreFileOnlyAsync`) uses the sender's StudyInstanceUID; empty → mints a fresh one. Series reuse same UID within the same study. No SOP collision check — same SOP across different studies is fine. Prints (`IngestPrintImageAsync`) generate fresh UIDs. Manual merge (`StudyService.MergeStudyAsync`) is the operator's explicit escape hatch. UIDs sanitized via `TruncateUid` (digits + dots only, max 64 chars). Per-study `_studyLocks` keyed on StudyInstanceUID. Tests: `UpsertIngestTests` pins grouping (`SameStudyUidTwoSeries_OneStudy`), separation (`DifferentStudyUidSamePatient_CreatesTwoStudies`), post-complete re-send (`ResendAfterComplete_CreatesNewStudy`).
