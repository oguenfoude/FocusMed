...
65. **`FocusMed.PrintService` is `net10.0-windows` (the third Windows-only project after `FocusMed.PrintCapture`)**. Required because `System.Drawing.Printing.PrintDocument` / `PrinterSettings.PaperSizes` are Windows-only APIs. It uses the **`PdfiumPrinter` 1.4.1** NuGet (which wraps Pdfium + `System.Drawing.Printing`) along with **`bblanchon.PDFium.Win32`** as the native runtime, and **`PdfSharpCore` 1.3.62` for booklet 2-up composition. **Does NOT use `System.Printing.PrintQueue.AddJob(path)`**: that path sends raw PDF bytes to a driver that may not understand PDF, which is the exact bug class that killed the prior SumatraPDF-based Dashboard printing. `WindowsDriverPrintService` opens the PDF with `PdfDocument.Load`, hands it to `CreatePrintDocument`, and raises the `PrintPage` event handler internally — pages are rasterized to bitmaps and drawn into `Graphics` scaled to `e.PageBounds`. Listens on **`http://localhost:5050` only** (no external). Dashboard talks to it via `PrintServiceClient` (`AddHttpClient<IPrintServiceClient, PrintServiceClient>`). CORS allows `localhost:5000` only. Configured printer validation happens at startup: missing queue is logged as ERROR but service stays up. **Auto-detection** scans `PrinterSettings.InstalledPrinters` to find matching Windows queues by display name, then picks the best driver automatically (native > generic class driver, `CanDuplex=true` preferred). Config entries only need a `Name` field — `WindowsQueueName` is optional override. `GET /printers/{name}/capabilities` returns `PrinterCapabilities` with `CanDuplex`, `SupportedDuplexModes`, and full `SupportedPaperSizes` from the live driver. `GET /printers` includes `CanDuplex` and `PaperSizeCount` per printer. **Booklet imposition** via `IBookletImpositionService` uses PdfSharpCore `XGraphics`+`XPdfForm` to compose two logical pages side-by-side on each physical sheet (standard saddle-stitch 2-up, padded to multiple of 4), then `WindowsDriverPrintService` prints the imposed PDF with `Duplex.Horizontal` (short-edge binding — REQUIRED for booklet fold). `PrintRequest.BookletMode=true` triggers imposition; blocked with clear error if printer does not report `Horizontal` in `SupportedDuplexModes`. Paper size for booklet is resolved from capability detection data, never hardcoded. Paper sizes are abstracted via `PaperSizePolicy` (defaults to A4, reads `config.PreferredPaperSize` if set). **Duplex detection** uses `PrinterSettings.CanDuplex` — `System.Printing.PrintQueue.GetPrintCapabilitiesAsXml()` was tried but throws "different thread owns it" because PrintQueue requires a UI dispatcher thread (STA) and ASP.NET runs on threadpool. When `CanDuplex=true`, both `Vertical` and `Horizontal` are reported as supported (matches every printer driver tested). Results are cached for 60s per printer.

66. **`PdfSharpCore` has `XGraphics`/drawing API — used for booklet 2-up composition.** PdfSharpCore 1.3.62 includes `PdfSharpCore.Drawing.XGraphics` + `XPdfForm` + `XBrush` + `XPen` + `XFont` + `XRect` + `DrawImage(form, XRect)`. `BookletImpositionService` (`src/FocusMed.PrintService/Services/BookletImpositionService.cs`) uses `XPdfForm.FromFile(sourcePdfPath)` + `form.PageNumber = pageIndex` + `gfx.DrawImage(form, new XRect(...))` to compose two logical pages side-by-side on each physical sheet (standard saddle-stitch 2-up imposition, signature order, padded to multiple of 4). The `multiplePages` parameter in `PdfPrintSettings` supports N-up grid printing (sequential pages) but cannot do booklet page reordering — that's why the imposition happens BEFORE handing the PDF to PdfiumPrinter.

67. **`PrinterSettings.CanDuplex` is driver-reported, not hardware truth.** The Brother MFC-J6720DW is a duplex-capable printer, but its Windows driver may report `CanDuplex = false` via `PrinterSettings`. Capability detection (`PrinterCapabilityDetector`) reads the driver's live value — it does not override or second-guess it. If `CanDuplex=false`, booklet mode is blocked with a clear error, and the duplex checkbox is disabled in the Dashboard UI. This is correct behavior: the driver is the authority on what the print subsystem can do.

68. **Print mode uses 3 capability-aware radio buttons** (`StudyDetails.razor:355-385`). The two old checkboxes (`PrintDuplex` + `PrintBooklet`) were replaced by a single radio group: "Recto simple" (simplex), "Recto-verso" (duplex long-edge), "Livret" (booklet). `PrintMode` (string, default `"simplex"`) is reset every time `OpenPrintDialog()` runs. `CanDuplex` + `HasShortEdge` are derived from `PrinterCapabilities.SupportedDuplexModes`. Disabled radios show tooltips explaining why: `(non disponible)` for duplex, `(recto-verso requis)` / `(reliure petit cote non supportee)` for booklet. `PrintRequest.Duplex` is computed as `PrintMode == "duplex" || PrintMode == "booklet"`; `BookletMode` as `PrintMode == "booklet"`.
...
- `src/FocusMed.PrintService/FocusMed.PrintService.csproj` — ASP.NET Core Minimal API (Microsoft.NET.Sdk.Web, net10.0-windows). Listens on `localhost:5050`. References `PdfiumPrinter` + `bblanchon.PDFium.Win32` + `PdfSharpCore`. Independent of Dicom/Worker/Data.
- `src/FocusMed.PrintService/Program.cs` — DI registration of `JobStateTracker` + `PrinterCapabilityDetector` + `BookletImpositionService` + `WindowsDriverPrintService`, 4 endpoint maps (Print, JobStatus, Printers, Capabilities), startup validation that checks configured Windows queue names against `PrinterSettings.InstalledPrinters` and logs capability data. CORS only allows `localhost:5000`.
- `src/FocusMed.PrintService/Abstractions/IPhysicalPrintService.cs` — interface with `GetCapabilitiesAsync` method.
- `src/FocusMed.PrintService/Abstractions/IBookletImpositionService.cs` — interface: `ComposeBookletAsync(sourcePdfPath, targetSheetSize, options)` returns path to imposed PDF.
- `src/FocusMed.PrintService/Abstractions/Dtos.cs` — `PrintRequest` (includes `BookletMode`), `PrintResult`, `JobStatus`, `PrinterInfo` (includes `CanDuplex`, `PaperSizeCount`), `PaperSizeInfo`, `PrinterCapabilities` records.
- `src/FocusMed.PrintService/Configuration/PhysicalPrinterOptions.cs` — POCO bound to `"PhysicalPrinters"` section. Each entry: `Name`, `WindowsQueueName` (exact `Get-Printer` output, case-sensitive), `Protocol`, `Enabled`, optional `PreferredPaperSize`.
...
- `src/FocusMed.PrintService/Services/PaperSizePolicy.cs` — abstracts paper-size selection. Defaults to A4, reads `config.PreferredPaperSize` if set.
- `src/FocusMed.PrintService/Services/PrinterCapabilityDetector.cs` — queries live Windows printer driver via `PrinterSettings` for `CanDuplex`, `PaperSizes`, and `Duplex` modes. Logs full result at Information level on every query.
- `src/FocusMed.PrintService/Services/BookletImpositionService.cs` — `IBookletImpositionService` implementation. Standard saddle-stitch 2-up imposition: uses `PdfSharpCore.Drawing.XGraphics` + `XPdfForm` to compose two logical pages side-by-side on each physical sheet (padded to multiple of 4). Signature order: Sheet k front = [page N-2k, page 2k+1], Sheet k back = [page 2k+2, page N-2k-1]. Writes imposed PDF to temp file.
- `src/FocusMed.PrintService/Services/WindowsDriverPrintService.cs` — `IPhysicalPrintService` implementation. Uses `PdfiumPrinter.PdfDocument.Load` + `CreatePrintDocument(PdfPrintSettings)` + binds `PrinterSettings.{Copies, Duplex, PaperSize}` directly. Booklet mode: calls `IBookletImpositionService.ComposeBookletAsync` first, then prints imposed PDF with duplex. Wraps everything in try/catch returning exact error messages (`ex.GetBaseException().Message`), never throws.
...
- `src/FocusMed.PrintService/Endpoints/PrintersEndpoint.cs` — GET `/printers` → `IReadOnlyList<PrinterInfo>` (includes CanDuplex, PaperSizeCount).
- `src/FocusMed.PrintService/Endpoints/CapabilitiesEndpoint.cs` — GET `/printers/{printerName}/capabilities` → `PrinterCapabilities` (live driver data: CanDuplex, SupportedDuplexModes, SupportedPaperSizes).
...
- `src/FocusMed.Dashboard/Services/PrintServiceClient.cs` — typed HttpClient wrapper around PrintService endpoints. `AddHttpClient<IPrintServiceClient, PrintServiceClient>` registered in `Dashboard/Program.cs`. BaseAddress from `"PrintService:BaseUrl"` config (default `http://localhost:5050`). Maps 4 endpoints to local records. Returns `PrintResult(false, null, "...")` on HttpRequestException / TaskCanceledException — never throws.
- `src/FocusMed.Dashboard/Components/Pages/StudyDetails.razor` — "Imprimer" pill button + modal (Copies 1–99 + Duplex toggle + Booklet toggle). Duplex checkbox shows "ON"/"OFF par defaut" label and is disabled with tooltip when `CanDuplex=false`. Booklet checkbox shows error if duplex unavailable. Loads capabilities via `GET /printers/{name}/capabilities` when dialog opens.
- `src/FocusMed.Dashboard/Components/Pages/Settings/Printers.razor` — `/settings/printers` page. Calls capabilities endpoint for each printer. Shows CanDuplex status, paper size count, and full list of supported paper sizes with dimensions. French UI.
