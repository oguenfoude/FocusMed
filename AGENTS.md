...
93. **C-STORE re-send visibility: `StoreOutcome` + dedupe log + no reopen on pure retry** (`DicomUpsertService.cs`, `FocusMedScp.cs`). An identical re-send (same SOP UIDs) used to hit a bare `return` with zero logging — sender saw Success, DB + logs + chime stayed silent, operator thought the send vanished. Now `StoreFileOnlyAsync` returns `StoreOutcome` (`Stored`/`DedupedSameStudy`/`SopCollisionForked`); a same-study duplicate logs `C-STORE deduped SOP=... AE=... — no new row` at Information and bumps ONLY `LastUpdatedAt` (row jumps to top, `Dernière réception` label moves) WITHOUT reopening `Complete`→`Receiving` (reopen happens only when new SOPs actually store). SOP collision across studies still forks the UID (Warning). `FocusMedScp` keeps per-association counters and logs `Association C-STORE summary AE=...: N stored, M deduped, K failed` on release/close — never `Failure` on a duplicate (would cause sender retry loops). Dashboard `MainLayout` shows `Dernière réception HH:mm:ss` from `MAX(LastUpdatedAt)` in the existing 2s poll; Home rows carry a `title` tooltip with last-activity time. Tests pin it (`Ingest_ResendAfterComplete_BumpsTimestampKeepsStatus`). Proven live: re-sent MEKKI/FARHAT studies produced `deduped` lines with image counts unchanged (2 and 4).
